...
export default function Hero({ openQuote }) {
  ...
  const [activeTab, setActiveTab] = useState('Marketing Digital');
      ...
      <div className="max-w-7xl mx-auto text-center relative w-full">
        <div className="max-w-7xl mx-auto flex flex-col lg:grid lg:grid-cols-2 text-center lg:text-left gap-6 sm:gap-8 lg:gap-12 items-center relative">
          ...
          <div className="relative order-2 w-full">
            <div className="relative bg-white/5 backdrop-blur-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-white/10">

              {/* Card interno */}
              <div className="bg-gradient-to-br from-gray-900/20 to-gray-800/20 backdrop-blur-sm rounded-lg border border-white/5 flex flex-col h-[320px] sm:h-[400px] lg:h-[480px] overflow-hidden">

                {/* HEADER das Tabs */}
                <div className="flex items-center justify-between px-4 py-2 bg-white/5 backdrop-blur-sm border-b border-white/10">
                ...
                {/* Tabs com rolagem */}
                <div className="flex overflow-x-auto scroll-smooth space-x-2 px-4 py-2 snap-x snap-mandatory">
                  {['Marketing Digital', 'Design Gráfico', 'Desenvolvimento Web', 'Suporte Técnico'].map((tab) => (
                    <button
                      key={tab}
                      className={`flex-shrink-0 px-4 py-2 text-sm rounded-lg border transition-all duration-200 whitespace-nowrap snap-start
                        ${activeTab === tab
                ...
                {/* Conteúdo do card */}
                <div className="flex-1 flex flex-col gap-4 px-4 py-3 overflow-y-auto">
                  {/* Imagem */}
                  ...
                  <p className="text-sm text-gray-200 leading-relaxed">
                    ...
                    {activeTab === 'Desenvolvimento Web' &&
                      'Desenvolvemos websites modernos, responsivos e otimizados para conversão, garantindo performance e segurança.'}
                    {activeTab === 'Suporte Técnico' &&
                      'Prestamos suporte técnico especializado, diagnósticos e planos estratégicos para garantir estabilidade e crescimento do seu negócio.'}
                  </p>
              ...
              {/* Floating Card */}
              <div
                className={`hidden lg:block absolute bottom-4 right-4 transform translate-x-6 translate-y-6 w-72 ${currentFloatingCard?.bgColor || 'bg-blue-500/20'} backdrop-blur-xl rounded-lg p-4 border border-white/10 shadow-2xl`}
              >
                <div className="flex items-center space-x-2 mb-2">
                  <div className={`w-6 h-6 ${currentFloatingCard?.iconColor || 'text-white'} flex items-center justify-center text-sm`}>
                    {currentFloatingCard?.icon || '💡'}
                  </div>
                  <span className={`text-sm font-medium ${currentFloatingCard?.textColor || 'text-white'}`}>
                    {currentFloatingCard?.title || 'Destaque'}
                  </span>
                </div>
                <div className={`text-sm text-left ${currentFloatingCard?.contentColor || 'text-gray-200'}`}>
